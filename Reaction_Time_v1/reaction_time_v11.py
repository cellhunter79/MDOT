...
chance = 2 # chance of displaying the bar
    ...
    for i in range(1,30,1):
        ...
        if rand_choice == 0:
            ...
            for m in range(1,4,1):
                ...
                Z_new = image_shuffle*Z_mask/255 + Z_signal
                np.save(('patterns/test'+str(i)+'_'+str(m)+'_signal'),np.array(Z_new))     
                image = np.load(('patterns/test'+str(i)+'_'+str(m)+'_signal.npy'))
                print('patterns/test'+str(i)+'_'+str(m)+'_signal.npy')
                im.set_data(image)
                plt.pause(0.1)
            ...
            for n in range(1,2,1):        
                image_shuffle = randomize_image(image_linearized, stim_x_size, stim_y_size)
                np.save(('patterns/test'+'_'+str(i)+'_'+str(n)+'_blank'),np.array(image_shuffle))
                image = np.load('patterns/test'+'_'+str(i)+'_'+str(n)+'_blank.npy')
                print(image.shape)
                im.set_data(image)
                plt.pause(0.1)
        rand_choice = round(np.random.random()*100)%chance
