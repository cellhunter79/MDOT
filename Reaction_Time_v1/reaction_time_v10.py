        ...
        for i in range(1,300,1):
            ...
            if rand_choice == 0:
                ...
                for m in range(1,10,1):
                    ...
                    Z_new_all = np.concatenate((Z_new_all,Z_new))
                    #np.save(('patterns/test'+str(i)+'_'+str(m)+'blank'),np.array(Z_new_all))     
                ...
                Z_new = randomize_image(image_linearized, stim_x_size, stim_y_size)
                Z_new_all = np.concatenate((Z_new_all,Z_new))
                #randomize_image(image_linearized, stim_x_size, stim_y_size)
                ...
                #im.set_data(image)
                #plt.pause(0.25)
            np.save(('patterns/test_all'),np.array(Z_new_all))  
            rand_choice = round(np.random.random()*100)%chance
            print(np.average(time_interval))
